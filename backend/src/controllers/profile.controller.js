...
export async function getProfile(req, res, next) {
  try {
    const { rows } = await query(
      `SELECT id, email, name, role, phone, location, street_address, delivery_preference, bio, avatar_url
  ...
  try {
    ...
    const { rows } = await query(
      `UPDATE users SET
         ...
         updated_at=NOW()
       WHERE id=$1
       RETURNING id, email, name, role, phone, location, street_address, delivery_preference, bio, avatar_url`,
      [req.user.id, name, phone, location, street_address, delivery_preference, bio, avatar_url]
    );
    res.json(rows[0]);
  } catch (e) { next(e); }
}
